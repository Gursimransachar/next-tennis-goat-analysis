...
matches_with_age AS (
  ...
  WHERE EXTRACT(YEAR FROM n.date) BETWEEN 2003 AND 2025
	AND ((n.Player_1 = n.winner AND CAST (n.Rank_2 AS INTEGER) < 11) OR
		(n.Player_2 = n.winner AND CAST (n.Rank_1 AS INTEGER) < 11))
),
filtered_matches AS (
  SELECT winner, generation, age, COUNT(*) AS wins
...
with_cumulative AS (
  SELECT 
    ...
    age,
    wins,
    SUM(wins) OVER (PARTITION BY winner ORDER BY age) AS cumulative_wins
